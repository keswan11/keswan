...
class Ptsp_pusat_lapangan extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('Ptsp_pusat_lapangan_model');
	...
	public function home(){
		...
		$menu = $this->Ptsp_pusat_lapangan_model->get_pengajuan_role_operator($id_role);
		$data = array(
			'title' => 'PTSP Pusat Lapangan',
			'menu' => $menu
		);
	...
	public function sip_drh_wni(){
		...
		$cek_page = $this->Ptsp_pusat_lapangan_model->cek_page($role);
		if($cek_page == false){
			...
			$data = array(
				...
				'data_surat_izin' => $this->Ptsp_pusat_lapangan_model->get_list_surat($role, 'surat_izin')
			);
	...
	public function surat_rekomendasi_sivet_ambulatori_wni(){
		...
		if($cek_page == false){
			...
			$data = array(
				...
				'data_surat_izin' => $this->Ptsp_pusat_lapangan_model->get_list_surat($role, 'surat_rekomendasi')
			);
			$this->template->load('templates/template_role','surat',$data);
		}
	}
	...
	public function detail_surat_izin(){
		if(!isset($_POST['submit']))
		{
			...
			$data['id_member']=$this->Ptsp_pusat_lapangan_model->get_member_by_pengajuan($id_pengajuan, 'izin');
			$data['data_pengajuan']=$this->Ptsp_pusat_lapangan_model->get_jenis_pengajuan($id_jenis_pengajuan);
			$data['id_jenis_pengajuan']=$id_jenis_pengajuan;
			$data['id_pengajuan']=$id_pengajuan;
			$data['data_pesyaratan']=$this->Ptsp_pusat_lapangan_model->get_persyaratan_form($id_jenis_pengajuan);
			$this->template->load('templates/template_role','detail',$data);

		}
		else
		{
			$this->Ptsp_pusat_lapangan_model->input_detail_surat_izin();
		}
	}
	...
	public function detail_surat_rekomendasi()
	{
		if(!isset($_POST['submit']))
		{
			...
			$id_jenis_pengajuan=$this->uri->segment(3);
			$id_pengajuan=$this->uri->segment(4);
			$data['id_member']=$this->Ptsp_pusat_lapangan_model->get_member_by_pengajuan($id_pengajuan,'rekomendasi');

			$data['data_pengajuan']=$this->Ptsp_pusat_lapangan_model->get_jenis_pengajuan($id_jenis_pengajuan);
			$data['id_jenis_pengajuan']=$id_jenis_pengajuan;
			$data['id_pengajuan']=$id_pengajuan;
			$data['data_kat_pesyaratan']=$this->Ptsp_pusat_lapangan_model->get_kat_persyaratan($id_jenis_pengajuan);
			$this->template->load('templates/template_role','detail_rekomendasi',$data);
		}
		else
		{
			// $this->db->trans_begin();
			$this->Ptsp_pusat_lapangan_model->input_detail_surat_rekomendasi();
		}
	}

	public function disposisi_surat(){
		$this->Ptsp_pusat_lapangan_model->update_status_pengajuan();
		redirect($this->uri->segment(1).'/home');
	}
}
